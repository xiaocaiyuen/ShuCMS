...
namespace Shu.Utility
{
    ...
    public class EKFileManager
    {
        #region 构造函数
        private static string strRootFolder;
        static EKFileManager()
        {
        ...
        /// 创建文件
        /// </summary>
        /// <param name="fileurlname">带路径的文件名</param>
        /// <param name="content">字符串内容</param>
        /// <returns>返回是否写入成功</returns>
        public static bool CreateFileContent(string fileurlname, string content)
        {
            FileInfo file = new FileInfo(fileurlname);
            file.Delete();
            using (FileStream fs = new FileStream(fileurlname, FileMode.OpenOrCreate))
            {
                lock (fs)
                {
                    if (!fs.CanWrite)
                    {
                        throw new System.Security.SecurityException("文件fileName=" + fileurlname + "是只读文件不能写入!");
                    }
