...
namespace Shu.BLL
{
    public class EKMessageBox
    {
        ...
        static EKMessageBox()
        {
            ...
            str_jboxres = "<link rel=\"stylesheet\" href=\"" + adminpath + "/tool/JBox/Skins/Blue/jbox.css\" />\n";
            str_jboxres += "<script type=\"text/javascript\" src=\"" + adminpath + "/js/jquery1.4.js\"></script>\n";
            str_jboxres += "<script type=\"text/javascript\" src=\"" + adminpath + "/tool/JBox/jquery.jBox-2.3.min.js\"></script>\n";
        }

        /// <summary>
        /// 判断使用情况
        /// </summary>
        private static void ModeType()
        {
            if (EKRequest.Request.Path.ToLower().Contains(MS_ConfigBLL.AdminPath.ToLower()))
            {
                mode = 1;//后台
            }
            else
        ...
        public static void Show(System.Web.UI.Page page, string msg)
        {
            ...
            if (mode == 0)
            {
                ...
                if (page.Header != null)
                {
                    page.Header.Controls.Add(new LiteralControl(str_script));
                }
                else
                {
                    System.Web.HttpContext.Current.Response.Write(str_script);
        ...
        protected static void JboxShow(string msg, string url)
        {
